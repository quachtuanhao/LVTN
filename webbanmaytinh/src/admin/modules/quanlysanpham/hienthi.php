...
<table class="content-wrapper" style="min-height: 395px;">
    ...
    if (isset($_GET['page'])) {
        ...
        $n = ($page - 1) * 5;
        $sql = $page == 1
            ? "SELECT maSanPham as id, tenSanPham as ten, gia, soLuong, hinhAnh as img, moTa, tenNhaSanXuat as tenNSX 
               FROM sanpham JOIN nhasanxuat ON sanpham.maNSX=nhaSanXuat.maNhaSanXuat 
               ORDER BY id ASC LIMIT 0, 5"
            : "SELECT maSanPham as id, tenSanPham as ten, gia, soLuong, hinhAnh as img, moTa, tenNhaSanXuat as tenNSX 
               FROM sanpham JOIN nhasanxuat ON sanpham.maNSX=nhaSanXuat.maNhaSanXuat 
               ORDER BY id ASC LIMIT $n, 5";
    } else {
        $sql = "SELECT maSanPham as id, tenSanPham as ten, gia, soLuong, hinhAnh as img, moTa, tenNhaSanXuat as tenNSX 
                FROM sanpham JOIN nhasanxuat ON sanpham.maNSX=nhaSanXuat.maNhaSanXuat 
                ORDER BY id ASC LIMIT 0, 5";
    }
    ...
    ?>
        <tr class="content-list" style="height: 70px;">
            ...
            <td class="content-item width150" style="height: 100%; overflow-y: hidden; max-height: 3em; overflow: hidden; text-overflow: ellipsis;"><?php echo $row['moTa']; ?></td>
