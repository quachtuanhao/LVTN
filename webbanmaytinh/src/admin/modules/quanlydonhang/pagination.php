<div style="width: 100%; display: flex; justify-content: center;">
    <div class="pagination" style="width: 600px; height: 30px; margin: 20px 0; display: flex;">
        <div class="prev" style="flex-grow: 1;">
            <?php if ($page > 1) { ?>
                <a href="./index.php?action=quanlydonhang&query=no&page=<?php echo $page - 1; ?>&text=<?php echo urlencode($searchText); ?>" 
        ...
        <div class="" style="flex-grow: 8;">
            <div style="display: flex; flex-direction: row;">
                <?php
                // Đếm tổng số đơn hàng với điều kiện tìm kiếm
                $countSql = "SELECT COUNT(maDonDatHang) FROM dondathang 
                             LEFT JOIN trangthai ON dondathang.maTT = trangthai.maTrangThai 
                             $searchCondition";
                $countResult = mysqli_query($conn, $countSql);
                $countRow = mysqli_fetch_assoc($countResult);
                $totalOrders = (int)$countRow['COUNT(maDonDatHang)'];
                ...
                for ($i = $start; $i <= $end; $i++) {
                ?>
                    <a href="./index.php?action=quanlydonhang&query=no&page=<?php echo $i; ?>&text=<?php echo urlencode($searchText); ?>" 
